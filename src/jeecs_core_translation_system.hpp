...
namespace jeecs
{
    struct TranslationUpdatingSystem :public game_system
    {
        ...
        void UpdateLocalToWorld(
            ...
            LocalToWorld* l2w)
        {
            l2w->pos = position ? *position : math::vec3();
            l2w->rot = rotation ? *rotation : math::quat();
            l2w->scale = scale ? *scale : math::vec3(1, 1, 1);
        }
        ...
        void UpdateLocalToParent(
            ...
            LocalToParent* l2P)
        {
            l2P->pos = position ? *position : math::vec3();
            l2P->rot = rotation ? *rotation : math::quat();
            l2P->scale = scale ? *scale : math::vec3(1, 1, 1);
        }
