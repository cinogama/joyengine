...
void fimg_seek(jeecs_fimg_file* file, int64_t offset, je_read_file_seek_mode mode)
{
    ...
    switch (mode)
    {
    case je_read_file_seek_mode::JE_READ_FILE_SEEK_SET:
        file->eof_flag = false;
        ...
        break;
    case je_read_file_seek_mode::JE_READ_FILE_SEEK_CURRENT:
        fimg_seek(
            file,
            (int64_t)file->nreaded_sz + offset,
            je_read_file_seek_mode::JE_READ_FILE_SEEK_SET);
        break;
    case je_read_file_seek_mode::JE_READ_FILE_SEEK_END:
        fimg_seek(
            file,
            (int64_t)file->f_size + offset,
            je_read_file_seek_mode::JE_READ_FILE_SEEK_SET);
        break;
    }

    for (auto fptr : file->fds)
        _je_file_seek(fptr, 0, je_read_file_seek_mode::JE_READ_FILE_SEEK_SET);

    if (file->fds.empty() == false)
        _je_file_seek(file->fds[0], file->f_diff_ptr, je_read_file_seek_mode::JE_READ_FILE_SEEK_SET);

    _je_file_seek(file->fds[file->nf_index], file->nf_diff_ptr, je_read_file_seek_mode::JE_READ_FILE_SEEK_SET);
}
