...
namespace jeecs
{
    class AudioContext
    {
        ...
        std::shared_mutex m_context_mx;

        std::mutex m_devices_mx;
        ...
        constexpr static ALCint M_ENGINE_ALC_MAX_AUXILIARY_SENDS = 8;
...
#undef JEAL_LOAD_FUNC
        ...
        void active_using_device(jeal_play_device* device)
        {
            ...
            // TODO: 在此处恢复Source, buffer, effect, effect slot, filter 等等
        ...
        // Buffer
        void update_buffer(jeal_buffer* buffer, bool restore_internal_states)
        {
            ...
            jeal_format format)
        {
            ...
            update_buffer(audio_buffer, false);
        ...
        void shutdown_buffer(const jeal_buffer* buffer)
        {
        ...
        // Source
        void update_source(jeal_source* source, bool restore_internal_states)
        {
            if (!m_current_play_device.has_value())
            {
                // 此时应该没有任何可用设备
    ...
    public:
        ...
        {
            // Remove from the set.
            m_buffers.erase(const_cast<jeal_buffer*>(buffer));
